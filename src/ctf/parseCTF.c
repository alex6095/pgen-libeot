...
enum EOTError parseCTF(struct Stream **streams, struct SFNTContainer **out)
{
  *out = NULL;
  ...
  struct SFNTTable *glyf = NULL, *loca = NULL, *maxp = NULL, *head = NULL, *hmtx = NULL, *hdmx = NULL, *VDMX = NULL, *cff = NULL;
  for (unsigned i = 0; i < (*out)->numTables; ++i)
  {
    ...
    else if (strncmp(tbl->tag, "CFF ", 4) == 0)
    {
      cff = tbl;
    }
  ...
  bool hasCff = (cff != NULL);
  if (!glyf && hasCff)
  {
    /* CFF-based OpenType fonts do not have glyf/loca tables. */
    return EOT_SUCCESS;
  }
  if (!loca)
  {
